# Profiles.json Documentation（属性说明文档）
...
| `wordDelimiters`              | Optional   | String  | ` /\()"'-:,.;<>~!@#$%^&*|+=[]{}~?│` *(`│` is `U+2502 BOX DRAWINGS LIGHT VERTICAL`)* | Determines the delimiters used in a double click selection.  |
...
| `name`                             | *Required* | String        |                 | Name of the profile. Displays in the dropdown menu. Additionally, this value will be used as the "title" to pass to the shell on startup. Some shells (like `bash`) may choose to ignore this initial value, while others (`cmd`, `powershell`) may use this value over the lifetime of the application. This "title" behavior can be overridden by using `tabTitle`. |
